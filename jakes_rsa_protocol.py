...
def test_rsa_works(P, Q, expon, expon_inv, message=111112222233333):
    ...
    print("\t\t P = %d" % P)
    print("\t\t Q = %d" % Q)
    print("\t\t expon_inv = (we don't technically need to store this since it can be computed as mult_inverse_mod(expon, (P - 1) * (Q - 1))")
    encrypted = rsa_encrypt_int(message, expon, P * Q)
    print("The encrypted message is %d" % encrypted)
    decrypted = rsa_decrypt_int(encrypted, expon_inv, P * Q)
    print("The decrypted message is %d" % decrypted)
    works = decrypted == message
    if works:
        print("RSA protocol worked!")
    else:
        print("Hmmm for some reason the RSA protocol failed")


if __name__ == '__main__':
    P, Q, expon, expon_inv = keygen(N_BITS)
    test_rsa_works(P,Q,expon,expon_inv)
